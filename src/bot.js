...
function issue(c,t){var npass=require('crypto').randomBytes(6).toString('hex');db.prepare('UPDATE tenant_config SET admin_password_hash=?, admin_password=NULL WHERE id=?').run(require('bcryptjs').hashSync(npass,10),t.id);return c.reply('🔑 '+t.name+'\nКод магазина: '+t.slug+'\nАдминка: https://zap.prostors.ru/admin.html?tenant='+t.slug+'\nВременный пароль: <code>'+npass+'</code>\nСмените его в Настройках после входа.\nВнимание: этот пароль заменяет все предыдущие.',{parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'📋 Скопировать пароль',copy_text:{text:npass}}]]}}).catch(function(){return c.reply('🔑 '+t.name+'\nКод магазина: '+t.slug+'\nАдминка: https://zap.prostors.ru/admin.html?tenant='+t.slug+'\nВременный пароль: '+npass+'\nСмените его в Настройках после входа.');});}
